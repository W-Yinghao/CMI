...
\begin{figure}[t]
  ...
  3A-R: original graph backbone \emph{fails} (near-chance, source-only) $\to$ 3A-S: known-good decoders
  \emph{pass} (protocol learnable; backbone was the bottleneck) $\to$ 3A-G: static-adjacency DGCNN
  \emph{learns the task} while dynamic-edge designs \emph{overfit} (consistent with a fingerprint channel,
  not a causal isolation) $\to$ 3A-H: graph/node audit \emph{clears the null} $\to$ 3A-I/J/K: fixed
  regularizer gives \emph{partial reduction at retention} on two datasets. \\[2pt]
