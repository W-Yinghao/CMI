...
# BNCI2014_001 full-LOSO seed0. Matched attribution: per (fold,backbone) train source-ERM M0 ONCE, adapt a COPY
# for erm_no_adapt (free M0 eval) + tta_control + cita_cmi lambda1.0 -> all from the IDENTICAL fold-M0 (no
# cross-run reproducibility assumption). Isolated out_dir gate_lambda1/ so the lambda=0.010 gate is untouched.
# Records loss-scale diagnostics (cond_domain_fraction_of_total_loss etc.) to prove the CMI term is ACTIVE.
# NO lambda grid, NO seeds 1/2, NO ConformerFull.
#SBATCH --job-name=cita-l1-bnci2014
#SBATCH --partition=A100,V100,V100-32GB,A40
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --output=logs/cita/cita-l1-bnci2014-%j.out
#SBATCH --error=logs/cita/cita-l1-bnci2014-%j.out
set -euo pipefail
