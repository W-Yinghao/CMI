...
# H-OOD stress world: lone concept site (frac 0.17) held out as target -> source concept-free.
# Harm is real but seed-idiosyncratic and NOT source-calibratable (gate_auroc always NaN).
# Kept as a TOS/support-refusal stress case, NOT valid as ACAR calibration evidence.
HARMFUL = {
...
# HFRAC = source-calibratable harmful world (H-CAL): concept_frac 0.5 -> ~3 concept sites, target
# hits one, ~2 remain in SOURCE so source pseudo-targets can exhibit TTA harm -> gate/ACAR gets a
# calibration signal. Primary harmful substrate for ACAR go/no-go.
HFRAC = {
    "HF1": dict(cov=0.8, prior=0.4, montage=0.2, concept=0.8, concept_frac=0.50),
    "HF2": dict(cov=0.8, prior=0.4, montage=0.2, concept=1.0, concept_frac=0.50),
    "HF3": dict(cov=0.8, prior=0.4, montage=0.2, concept=1.2, concept_frac=0.50),
    "HF4": dict(cov=1.2, prior=0.4, montage=0.2, concept=1.0, concept_frac=0.50),
    "HF5": dict(cov=0.8, prior=0.8, montage=0.2, concept=1.0, concept_frac=0.50),
    "HF6": dict(cov=1.2, prior=0.8, montage=0.3, concept=1.0, concept_frac=0.50),
}

_TABLES = {"recoverable": RECOVERABLE, "harmful": HARMFUL, "hfrac": HFRAC}

SUMMARY_COLS = [
    "world", "config", "seed", "epochs", "sites", "subjects", "sessions", "trials",
    "cov", "prior", "montage", "noise", "concept", "concept_frac",
    "target_sites", "concept_sites", "num_concept_sites", "source_concept_count", "target_concept_hit",
    ...
    "gate_auroc", "gate_auroc_is_finite", "gate_n_pseudo",
    "pseudo_harm_count", "pseudo_nonharm_count", "pseudo_gain_min", "pseudo_gain_mean",
    "pseudo_gain_max", "pseudo_gain_has_two_classes",
    "leakage_site_excess", "leakage_subject_excess", "leakage_session_excess",
]


def _params(world: str, cid: str) -> dict:
    table = _TABLES.get(world)
    if table is None or cid not in table:
...
def reconstruct_source_concept_count(n_sites: int, concept_frac: float, seed: int) -> int:
    """#concept-shifted sites remaining in SOURCE after holding out the target site."""
    cs = set(reconstruct_concept_sites(n_sites, concept_frac, seed))
    return len(cs - {reconstruct_target_site(n_sites, seed)})
...
def run_one(world: str, cid: str, seed: int, out_dir: str, leakage_nperm: int = 20) -> dict:
    ...
    from h2cmi.eval.harness import (evaluate_strict_dg, train_safety_gate,
                                    evaluate_offline_tta, evaluate_online_tta)
    ...
    # Replicate run_three_settings() but CAPTURE the gate object (its train_gains_ carries the
    # per-pseudo-target gains we need for the source-calibration diagnostics). Same public harness
    # functions, same order/args as h2cmi.eval.harness.run_three_settings -> identical results.
    strict = evaluate_strict_dg(model, Xt, yt, tgt_unit, device=cfg.train.device, prior=None)
    gate, gate_info = train_safety_gate(model, Xs, ys, None, cfg, src_unit, pi_star,
                                        device=cfg.train.device)
    offline = evaluate_offline_tta(model, Xt, yt, tgt_unit, cfg, pi_star,
                                   gate=gate, device=cfg.train.device)
    online = evaluate_online_tta(model, Xt, yt, tgt_unit, cfg, pi_star, device=cfg.train.device)
    results = dict(strict_dg=strict, gate_info=gate_info, offline_tta=offline, online_tta=online)
    ...
    num_concept_sites = int(round(p["concept_frac"] * COMMON["sites"]))
    source_concept_count = len(set(concept_sites) - set(tgt_sites))

    # SOURCE-calibration signal: per-pseudo-target gains from the fitted gate (train_gains_).
    # This is the crux for ACAR: if source pseudo-targets are a single harm-class, no harm can
    # be calibrated (gate_auroc = NaN) regardless of target harm.
    pg = getattr(gate, "train_gains_", None)
    if pg is not None and len(pg) > 0:
        pg = np.asarray(pg, dtype=float)
        harm_lab = (pg < -cfg.gate.harm_delta).astype(int)      # harm_delta=0 -> harm iff gain<0
        pseudo_harm_count = int((harm_lab == 1).sum())
        pseudo_nonharm_count = int((harm_lab == 0).sum())
        pseudo_gain_min, pseudo_gain_mean, pseudo_gain_max = float(pg.min()), float(pg.mean()), float(pg.max())
        pseudo_two = bool(np.unique(harm_lab).size >= 2)
    else:                                                       # gate not fit (n_pseudo<2): reason-code, don't fake
        pseudo_harm_count = pseudo_nonharm_count = 0
        pseudo_gain_min = pseudo_gain_mean = pseudo_gain_max = float("nan")
        pseudo_two = False
    hm = results["gate_info"].get("harm_metrics", {})
    gate_auroc = float(hm.get("auroc", float("nan")))
    gate_auroc_is_finite = bool(np.isfinite(gate_auroc))
    ...
    row = dict(
        ...
        num_concept_sites=num_concept_sites, source_concept_count=source_concept_count,
        target_concept_hit=hit,
        ...
        gate_auroc=gate_auroc, gate_auroc_is_finite=gate_auroc_is_finite,
        gate_n_pseudo=int(results["gate_info"]["n_pseudo"]),
        pseudo_harm_count=pseudo_harm_count, pseudo_nonharm_count=pseudo_nonharm_count,
        pseudo_gain_min=pseudo_gain_min, pseudo_gain_mean=pseudo_gain_mean,
        pseudo_gain_max=pseudo_gain_max, pseudo_gain_has_two_classes=pseudo_two,
    ...
    # report mirrors run_synthetic's core report + split facts + source-calibration (deliverable)
    report = dict(
        config=dict(world=world, cid=cid, seed=seed, classes=COMMON["classes"],
                    ...
                    target_sites=tgt_sites, num_concept_sites=num_concept_sites,
                    source_concept_count=source_concept_count, target_concept_hit=hit),
        source_calibration=dict(
            pseudo_harm_count=pseudo_harm_count, pseudo_nonharm_count=pseudo_nonharm_count,
            pseudo_gain_min=pseudo_gain_min, pseudo_gain_mean=pseudo_gain_mean,
            pseudo_gain_max=pseudo_gain_max, pseudo_gain_has_two_classes=pseudo_two,
            gate_auroc=gate_auroc, gate_auroc_is_finite=gate_auroc_is_finite,
            pseudo_gains=(pg.tolist() if pg is not None else None)),
    ...
    print(f"[run] {world}/{cid}/seed{seed}: strict={row['strict_bacc']:.3f} "
          f"d_off={row['offline_delta_bacc']:+.3f} hit={hit} src_concept={source_concept_count} "
          f"pseudo_harm={pseudo_harm_count}/{pseudo_harm_count + pseudo_nonharm_count} "
          f"auroc_finite={gate_auroc_is_finite} -> {path}")
    return row
...
def _two_classes(r: dict) -> bool:
    return bool(r.get("pseudo_gain_has_two_classes")) or bool(r.get("gate_auroc_is_finite"))


def _hfrac_pass(rows: list[dict]) -> bool:
    """Source-calibratable harmful world (H-CAL). Requires target harm AND that source
    pseudo-targets are non-degenerate (2 harm-classes) so ACAR/gate can calibrate."""
    d = [r["offline_delta_bacc"] for r in rows]
    n = len(rows)
    almost_all = max(1, n - 1)                                  # ">=4/5" for n=5
    return (all(r["target_concept_hit"] for r in rows)
            and all(int(r.get("source_concept_count", 0)) >= 1 for r in rows)
            and _mean([r["strict_bacc"] for r in rows]) >= 0.55
            and _mean(d) <= -0.05
            and sum(1 for x in d if x < 0) >= almost_all
            and _mean([r["offline_p_gt0"] for r in rows]) <= 0.40
            and sum(1 for r in rows if _two_classes(r)) >= almost_all)
...
def aggregate(run_dir: str, out_csv: str) -> None:
    ...
    print("\n===== per-config summary =====")
    _PASS = {"recoverable": _recoverable_pass, "harmful": _harmful_pass, "hfrac": _hfrac_pass}
    passed = {"recoverable": [], "harmful": [], "hfrac": []}
    for (world, cid), rs in sorted(groups.items()):
        ...
        two = sum(1 for r in rs if _two_classes(r))            # source-calibratable seeds
        scc = _mean([int(r.get("source_concept_count", 0)) for r in rs])
        ok = _PASS.get(world, _harmful_pass)(rs)
        print(f"  {world:11s} {cid}  n={len(rs)}  strict={mb:.3f}  d_off={md:+.3f}  "
              f"p_gt0={mp:.2f}  hit={hits}/{len(rs)}  neg={neg}/{len(rs)}  "
              f"src_concept={scc:.1f}  2class={two}/{len(rs)}  PASS={ok}")
        if ok:
            passed[world].append((md, mb, cid, rs))

    print("\n===== best configs =====")
    rp = passed["recoverable"]
    if rp:
        rp.sort(key=lambda t: (-t[0], -t[1]))                 # biggest reliable gain, then higher strict
        md, mb, cid, rs = rp[0]
        print(f"  BEST RECOVERABLE = {cid}  d_off={md:+.3f} strict={mb:.3f} seeds={[r['seed'] for r in rs]}")
    else:
        print("  BEST RECOVERABLE = NONE PASSED (closest in table above)")
    hp = passed["hfrac"]                                       # H-CAL: primary harmful substrate for ACAR
    if hp:
        hp.sort(key=lambda t: (t[0], -t[1]))                  # most harm (most negative), then higher strict
        md, mb, cid, rs = hp[0]
        print(f"  BEST HARMFUL-CAL = {cid}  d_off={md:+.3f} strict={mb:.3f} seeds={[r['seed'] for r in rs]}")
    else:
        print("  BEST HARMFUL-CAL = NONE PASSED (closest in table above)")
    if passed["harmful"]:
        md, mb, cid, rs = sorted(passed["harmful"], key=lambda t: (t[0], -t[1]))[0]
        print(f"  (legacy harmful  = {cid}  d_off={md:+.3f} strict={mb:.3f})")
...
def selftest() -> None:
    ...
    checks = 0
    ns = COMMON["sites"]
    for cid, p in list(HARMFUL.items())[:2] + list(HFRAC.items())[:2] + list(RECOVERABLE.items())[:1]:
        for seed in range(6):
            shift = ShiftSpec(cov=p["cov"], prior=p["prior"], concept=p["concept"],
                              concept_site_frac=p["concept_frac"], montage=p["montage"],
                              noise=COMMON["noise"], label_mechanism_rho=COMMON["label_rho"])
            ...
            real_scc = len(set(real_con) - set(real_tgt))
            assert real_tgt == [reconstruct_target_site(ns, seed)], (cid, seed, real_tgt)
            assert real_con == reconstruct_concept_sites(ns, p["concept_frac"], seed), (cid, seed, real_con)
            assert real_scc == reconstruct_source_concept_count(ns, p["concept_frac"], seed), (cid, seed, real_scc)
            checks += 1
    print(f"[selftest] RNG split + source_concept_count reconstruction matches the real "
          f"simulator on {checks} (config,seed) pairs OK")


def prescan(cid: str, nseeds: int, need: int, world: str | None = None) -> None:
    world = world or _world_of(cid)
    p = _params(world, cid)
    ns, frac = COMMON["sites"], p["concept_frac"]
    # H-CAL requires target harm AND source concept sites (so the gate can calibrate)
    qualifying = [s for s in range(nseeds)
                  if target_concept_hit(ns, frac, s) and reconstruct_source_concept_count(ns, frac, s) >= 1]
    chosen = qualifying[:need]
    num_cs = int(round(frac * ns))
    detail = {s: reconstruct_source_concept_count(ns, frac, s) for s in chosen}
    print(f"[prescan] {world}/{cid} frac={frac} n_sites={ns} num_concept_sites={num_cs}: "
          f"{len(qualifying)}/{nseeds} qualifying (target_hit & source_concept>=1); "
          f"first {need} = {chosen}  source_concept_count={detail}")
...
def main() -> None:
    ...
    ap.add_argument("--world", choices=["recoverable", "harmful", "hfrac"])
    ...
    if args.mode == "selftest":
        ...
        prescan(args.config, args.nseeds, args.need, world=args.world)
    elif args.mode == "run":
