#!/bin/bash
# Cross-Session 5-arm continuation training -- 63-bundle FAIL-RESUMABLE SLURM ARRAY. From one hash-verified ERM
...
#   # after 63/63: python scripts/aggregate_cs_arms.py --from-dir results/cmi_trace_cs_arms --expect 63
#SBATCH --job-name=cs-arms
...
#SBATCH --mem=32G
#SBATCH --output=logs/cs/cs-arms-%A_%a.out
#SBATCH --error=logs/cs/cs-arms-%A_%a.out
set -euo pipefail
...
OUTDIR="results/cmi_trace_cs_arms"
...
"$PY" -m scripts.run_cs_arms --bundle-index "$IDX" --device cuda --out-dir "$OUTDIR" \
  --cache-dir results/cmi_trace_mcc/warmup_cache --verify-warmup-from results/cmi_trace_mcc \
  --warmup-epochs 300 --cont-epochs 20 --K 4 --lam 1.0 --ramp 5 --cont-lr 1e-4
echo "[cs-arms] bundle $IDX complete"
