#!/bin/bash
# CIGL_69A2 (b) — conformer_full ERM full-LOSO seed0 on BNCI2014_001 (9 folds = 9 GPU runs).
# ISOLATED out_dir metacmi_gate_full/ so the frozen CIGL_69A evidence (metacmi_gate/) is NEVER touched or
# clobbered. conformer_full ONLY -> exactly 9 runs; eegnet/conformer ERM are reused from CIGL_69A at readout.
# Same protocol/preprocessing/split/audit as CIGL_69A (epochs 80, probe 100, n_perm 50, seed 0).
#SBATCH --job-name=metacmi-full-erm-bnci2014
#SBATCH --partition=A100,V100,V100-32GB,A40
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --output=logs/cigl/metacmi-full-erm-bnci2014-%j.out
#SBATCH --error=logs/cigl/metacmi-full-erm-bnci2014-%j.out
set -euo pipefail
