...
def main():
    ...
    overall = {ds: five_gate_verdict(next(c["gate1"] for c in cluster_rows if c["dataset"] == ds),
                                     next(c["gate2"] for c in cluster_rows if c["dataset"] == ds),
                                     next(c["gate3"] for c in cluster_rows if c["dataset"] == ds), g4,
                                     next(c["gate5"] for c in cluster_rows if c["dataset"] == ds)) for ds in datasets}
    ...
    json.dump({"per_dataset_gates": {c["dataset"]: {k: c[k] for k in ("gate1", "gate2", "gate3", "gate5", "di_specific_lcb")} for c in cluster_rows},
    ...
    for ds in datasets:
        ...
        print(f"     gates obs={c['gate1']} act={c['gate2']} rec={c['gate3']} safe={g4} spec={c['gate5']} -> {overall[ds]['outcome']}")
