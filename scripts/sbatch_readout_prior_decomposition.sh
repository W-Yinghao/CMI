...
#
#   sbatch --array=0-251 scripts/sbatch_readout_prior_decomposition.sh
#   # after 252/252: python scripts/aggregate_readout_prior_decomposition.py --from-dir results/cmi_trace_readout_prior --expect 252
#SBATCH --job-name=ro-prior
#SBATCH --partition=CPU
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --output=logs/rp/ro-prior-%A_%a.out
#SBATCH --error=logs/rp/ro-prior-%A_%a.out
set -euo pipefail
cd "${SLURM_SUBMIT_DIR:-$(pwd)}"
mkdir -p logs/ro results/cmi_trace_readout_prior/cells
...
"$PY" -m scripts.run_readout_prior_decomposition --cell-index "$IDX" --out-dir "$OUTDIR" --n-random 50 --n-draws 50
echo "[ro-prior] cell $IDX complete"
