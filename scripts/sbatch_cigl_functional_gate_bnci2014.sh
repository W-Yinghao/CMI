#!/bin/bash
#SBATCH --job-name=fcigl-gate-bnci2014
#SBATCH --partition=A100,V100,V100-32GB,A40
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --output=logs/cigl/fcigl-gate-bnci2014-%j.out
#SBATCH --error=logs/cigl/fcigl-gate-bnci2014-%j.out
set -euo pipefail
...
"$PY" scripts/run_cigl_functional_gate.py --dataset BNCI2014_001 --device cuda --seed "${GATE_SEED:-0}" \
  --epochs 80 --probe_epochs 100 --n_perm 50 --fcigl_k 2 --fcigl_update_every 10 --methods ${FGATE_METHODS:-fcigl_align_eta0.01 fcigl_align_eta0.05}
