...
while :; do
  ...
  echo "[cs-driver] done=$done_n/63 $(date -u +%H:%M:%S)"
  [ "$done_n" -ge 63 ] && { echo "[cs-driver] all 63 bundles done"; break; }
  missing=""
  for i in $(seq 0 62); do [ -f "$OUTDIR/${keys[$i]}.done" ] || missing="$missing $i"; done
  missing=$(echo "$missing" | tr ' ' '\n' | grep -v '^$')
  mine=$(squeue -u "$USER" -h 2>/dev/null | grep -c cs-arms || true)
  room=$((CAP - mine))
  if [ "$room" -ge 1 ] && [ -n "$missing" ]; then
    chunk=$(echo "$missing" | head -n "$room" | paste -sd, -)
    if [ -n "$chunk" ]; then
      if sbatch --array="$chunk" scripts/sbatch_cs_arms.sh >/tmp/cs_sb.$$ 2>&1; then
        echo "[cs-driver] submitted missing [$chunk] ($(cat /tmp/cs_sb.$$))"
      else
        echo "[cs-driver] submit refused: $(cat /tmp/cs_sb.$$)"
      fi
    fi
  fi
  sleep 90
