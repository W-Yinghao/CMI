"""CSC B8.1 canary worker (development-only, NO tag). Per cohort: build a class-balanced randomized-audit world
(contract-satisfying or contract-violating) on Lee2019 SM16 geometry with a PRE-ASSIGNMENT design_class (=real cued
class y0) + a hash-pinned registered randomization schedule; run b8_1_certify (hard provenance gate, contract-first
refusal + EXACT (block, design_class) randomization null). Records the B8 state + provenance/contract diagnostics.
Fresh base 420e6. Fail-closed."""
...
def run_cohort(world, r, base):
    ...
    try:
        ...
        Z, Y, C, G, Block, Dc, C_table, table_hash, contract_intended = B8.build_b8_1_cohort(world, coh, rng)
        rr = B8.b8_1_certify(Z, Y, C, G, Block, Dc, C_table, table_hash, m=len(subj), seed=seed, n_boot=IF.ML["n_boot"])
        st = str(rr.get("b8_state"))
        alert = (st == "B8_CONCEPT_ALERT")
        auc = _f(rr.get("contract_within_block_C_Z_auc"))
        rec = dict(world=world, world_index=idx, world_class=kind, has_concept=bool(has_concept),
                   ...
                   auc_le_tau=bool(auc == auc and auc <= B8.TAU_CONTRACT_AUC),  # AUC-only gate would PASS this cohort
                   ...
                   contract_within_block_C_Z_auc=auc,
                   contract_n_support_blocks=int(rr.get("contract_n_support_blocks", 0) or 0),
                   contract_n_strata=int(rr.get("contract_n_strata", 0) or 0),
                   contract_max_margin=_f(rr.get("contract_max_margin")),
...
if __name__ == "__main__":
    ...
    print(f"[b8.1 {a.base}:{a.world}:{a.start}+{a.n}] {len(recs)} recs, {len(errs)} err | alert={sum(1 for r in recs if r.get('b8_alert'))} "
          f"valid={sum(1 for r in recs if r.get('contract_valid'))} | states={dict(sc)}", flush=True)
