# CIGL_60 (R1) — Evidence hardening + multi-probe stress audit (non-GPU scaffold)
...
Unit tests / synthetic fixtures validate:
  implementation correctness, path safety, p-value / FDR / bootstrap behavior, calibration utilities.

They do NOT validate:
  CIGL effectiveness, leakage reduction on EEG, model-level improvement, mechanism claims.

All scientific claims require real EEG full-LOSO results.
```

Branch `project/cigl-r123-scaffold` off the CIGL audit line `fb5761d`. Implements the CIGL_45 R1 program:
build the machinery to make "graph/node leakage exists and CIGL reduces it" **statistically defensible** and
**probe-family robust** — the scientific verdict comes only from real EEG runs. Code + tests only, **no GPU**.
...
- **The 7-probe audit interface is WIRED** (over frozen features + null + FDR). The full 7-probe *agreement
  criterion* (≥5/7 on real EEG) is **pending real-EEG execution** — the phase-3A audit today still runs only a
  single 1-layer MLP; R1 makes the multi-probe path available, it does not yet establish the multi-probe result.
