...
- **Artifacts:** `notes/PHASE21_LPC_COLLAPSE_MECHANISM.md` + `results/tos_cmi_eeg_frozen/lpc_collapse_curves/collapse_curves.png` (the numeric per-backbone JSONs are **not** committed on `tos` — values are in PHASE21 + `CLAIMS_LEDGER.md` C5). Branch `tos` @ 1c65d79.
...
- **Artifacts:** `CLAIMS_LEDGER.md` C6 (the `variant_compare.json` cited in the TOS ledger is **not** committed on `tos`; the C6 numbers are in CLAIMS_LEDGER). Branch `tos` @ 1c65d79.
...
**Ladder-coverage note (revised Phase-1 gate, 2026-07-06).** The Phase-1 quantitative-inclusion gate (FSR_00 §10, PM patch) admits a route to a cross-level test only if it has ≥1 predictor-side level {L1,L2,L3,L4} **and** ≥1 endpoint-side level {L5,L6}; otherwise it is tagged `SUPPORT_ONLY` / `BOUNDARY_ONLY` / `PROTOCOL_ONLY` / `BACKGROUND_ONLY` and may inform interpretation but not RQ1–RQ3 regression. Under this gate the CMI-control cluster (predictor L1/L2/L4 + endpoint L5/L6) is quantitatively includable; the TOS erasure operators (predictor L3/L4 + endpoint L6) are includable for RQ2; but the pure diagnostics are `SUPPORT_ONLY`: FBCSP branch-ablation (L4 branch-load, no L1/L5 pairing — coincides exactly with the one HIGH missing metric, the per-branch leakage probe, §7), the refusal gate (L1 + decision, no endpoint), and the FBCSP bottleneck/graph-starvation routes (L4-only). This is recorded, not papered over, and the exact inclusion decision per route is machine-enumerated in `results/fsr_phase2/analysis_inclusion_table.csv` (Step 2A).
