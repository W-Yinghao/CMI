...
def _excess(v, chance):
    """bAcc above chance (chance = 1/n_classes). None if either operand is missing."""
    return round(v - chance, 4) if (isinstance(v, (int, float)) and chance is not None) else None


def _excess_norm(v, chance):
    """(bAcc - chance)/(1 - chance): 0 at chance, 1 at perfect — comparable across class counts."""
    if isinstance(v, (int, float)) and chance is not None and chance < 1.0:
        return round((v - chance) / (1.0 - chance), 4)
    return None


def _gain_norm(g, chance):
    """A bAcc GAIN normalized by the headroom (1 - chance), so 2- and 4-class gains compare."""
    if isinstance(g, (int, float)) and chance is not None and chance < 1.0:
        return round(g / (1.0 - chance), 4)
    return None
...
def load_run(run_dir) -> Dict[str, Any]:
    ...
    # chance-normalized metrics: bAcc excess over chance (1/n_classes) and headroom-normalized
    # versions, so 2-class and 4-class runs can be compared across datasets without pooling raw bAcc.
    nclass = manifest.get("n_classes")
    chance = (1.0 / nclass) if isinstance(nclass, int) and nclass > 0 else None
    strict, online = out["strict_dg_bacc"], out["online_tta_bacc"]
    gain = out["offline_tta_gain_bacc"]
    out.update({
        "n_classes": nclass,
        "chance_bacc": round(chance, 4) if chance is not None else None,
        "strict_dg_bacc_excess": _excess(strict, chance),
        "strict_dg_bacc_excess_norm": _excess_norm(strict, chance),
        "online_tta_bacc_excess": _excess(online, chance),
        "online_tta_bacc_excess_norm": _excess_norm(online, chance),
        "offline_tta_gain_bacc_norm": _gain_norm(gain, chance),
    })
    return out
...
def _group_stats(ok: List[Dict[str, Any]], key: str) -> Dict[str, Any]:
    ...
    return {k: {"n_ok": len(rs),
                ...
                "strict_dg_bacc_excess_norm": _stats([x.get("strict_dg_bacc_excess_norm") for x in rs]),
                "offline_tta_gain_bacc": _stats([x.get("offline_tta_gain_bacc") for x in rs]),
                "offline_tta_gain_bacc_norm": _stats([x.get("offline_tta_gain_bacc_norm") for x in rs]),
                "online_tta_bacc": _stats([x.get("online_tta_bacc") for x in rs]),
                "offline_tta_harm_rate": _harm_rate(rs)}
            for k, rs in sorted(groups.items())}


def _uniform_n_classes(ok: List[Dict[str, Any]]):
    """The single n_classes shared by all ok runs, or None if absent/mixed (within-dataset digest
    should be single-valued; a mixed value signals a corrupted grid root)."""
    vals = {r.get("n_classes") for r in ok if isinstance(r.get("n_classes"), int)}
    return next(iter(vals)) if len(vals) == 1 else None


def aggregate(runs: List[Dict[str, Any]], expected_targets=None,
              expected_seeds=None, expected_cells=None) -> Dict[str, Any]:
    ...
    agg = {
        ...
        # chance-normalized means (the cross-dataset-comparable numbers)
        ...
        "overall": {
            ...
            "strict_dg_bacc_excess_norm": _stats([r.get("strict_dg_bacc_excess_norm") for r in ok]),
            "offline_tta_gain_bacc": _stats(gains),
            "offline_tta_gain_bacc_norm": _stats([r.get("offline_tta_gain_bacc_norm") for r in ok]),
            "online_tta_bacc": _stats([r.get("online_tta_bacc") for r in ok]),
            "online_tta_bacc_excess_norm": _stats([r.get("online_tta_bacc_excess_norm") for r in ok]),
    ...
    # expected-grid completeness (a MISSING cell — no dir at all — is not a legal skip). Prefer an
    # explicit expected_cells list (from grid_manifest.json); else the target×seed product.
    expected = None
    if expected_cells is not None:
        expected = {(str(c["target_subject"]), str(c["seed"])) for c in expected_cells}
    elif expected_targets is not None and expected_seeds is not None:
...
def build_summary(root, project="Project A", step="Step 8", dataset="BNCI2014_001",
                  expected_targets=None, expected_seeds=None, expected_cells=None) -> Dict[str, Any]:
    runs = index_runs(root)
    return {
        "project": project, "step": step, "dataset": dataset, "root": str(root),
        "n_runs": len(runs), "runs": runs,
        "aggregate": aggregate(runs, expected_targets, expected_seeds, expected_cells),
...
def write_summary_md(summary: Dict[str, Any], path) -> str:
    ...
    lines = [f"# {summary['step']} {summary['dataset']} audited mini-grid summary", "",
             ...
             f"- n_classes: **{a.get('n_classes')}**  ·  chance bAcc: **{a.get('chance_bacc')}**  ·  "
             f"mean strict-DG excess-norm: **{a.get('mean_strict_dg_bacc_excess_norm')}**  ·  "
             f"mean offline-TTA gain-norm: **{a.get('mean_offline_tta_gain_bacc_norm')}**",
