...
def route_units(rows, deployable=DEPLOYABLE) -> list[dict]:
    ...
    for (diff, scen, seed, site), vmap in units.items():
        ...
        if scores.get(selected, 0.0) <= 0.0:                   # fixed rollback rule
            selected = IDENTITY
        id_bacc = vmap[IDENTITY].get(OOF)
        sel_bacc = vmap[selected].get(OOF)
        # oracle-best among the DEPLOYABLE actions actually present
        cand = {a: vmap[a].get(OOF) for a in deployable if a in vmap and _finite(vmap[a].get(OOF))}
        oracle_act = max(cand, key=cand.get) if cand else IDENTITY
        oracle_bacc = cand.get(oracle_act, id_bacc)
        d_router = (sel_bacc - id_bacc) if (_finite(sel_bacc) and _finite(id_bacc)) else float("nan")
        out.append(dict(difficulty=diff, scenario=scen, data_seed=seed, target_site=site,
                        selected=selected, adapted=(selected != IDENTITY),
                        d_router=d_router,
                        regret=(oracle_bacc - sel_bacc) if (_finite(oracle_bacc) and _finite(sel_bacc)) else float("nan"),
                        top1_agree=(selected == oracle_act),
...
def router_report(rows, *, deployable=DEPLOYABLE, n_boot=10000) -> dict:
    ...
    for diff in sorted({u["difficulty"] for u in units}):
        ...
        # seed-clustered ΔbAcc on SHIFT scenarios (where adaptation can help)
        mean, lo, hi, ns = _cluster_bootstrap(_by_seed(shift, "d_router"), n_boot=n_boot)
        rmean, rlo, rhi, _ = _cluster_bootstrap(_by_seed(shift, "regret"), n_boot=n_boot)
        d_all = np.array([u["d_router"] for u in du if _finite(u["d_router"])])
        ...
        rep["by_difficulty"][diff] = dict(
            n_units=len(du),
            delta_bacc_router_shift=dict(mean=mean, ci_lo=lo, ci_hi=hi, n_seeds=ns),
            action_regret_shift=dict(mean=rmean, ci_lo=rlo, ci_hi=rhi),
            harm_rate=float(np.mean(d_all < -1e-9)) if len(d_all) else float("nan"),
            mean_delta_bacc_all=float(np.nanmean([u["d_router"] for u in du])),
            ...
            top1_oracle_agreement=float(np.mean([u["top1_agree"] for u in du])),
...
def main():
    ...
    for diff, d in rep["by_difficulty"].items():
        db = d["delta_bacc_router_shift"]
        print(f"[{diff}] n={d['n_units']}  ΔbAcc_router(shift)={db['mean']:+.3f} "
              f"[{db['ci_lo']:+.3f},{db['ci_hi']:+.3f}]  harm={d['harm_rate']:.2f}  "
              f"regret={d['action_regret_shift']['mean']:.3f}  "
              f"false_adapt_null={d['false_adaptation_rate_null']:.2f}  "
              f"adapt_shift={d['adaptation_rate_shift']:.2f}  "
              f"top1_oracle={d['top1_oracle_agreement']:.2f}  "
