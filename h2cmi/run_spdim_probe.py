...
def _write_audit(path: str, *, args, rows: list[dict], command: str, started: float, finished: float,
    ...
    lines.extend([
        ...
        f"- expected_ok_rows_for_success: `{2 * 4}`",
    ])
...
def _run_target(ep, target: int, *, args, bn, TSMNet, Trainer, runner_commit: str, external_sha: str):
    ...
    y_true, y_pred = _predict(model, eval_loader, device=device, dtype=dtype, parameter_t=parameter_t)
    source_eval_seconds = time.time() - eval_start
    rows.append(meta | _metrics(y_true, y_pred) | {
        "mode": "source_only",
    ...
    eval_start = time.time()
    y_true, y_pred = _predict(rct_model, eval_loader, device=device, dtype=dtype, parameter_t=parameter_t)
    rows.append(meta | _metrics(y_true, y_pred) | {
        "mode": "rct_refit",
        "parameter_t": float(parameter_t.item()),
        "source_model_sha256": source_model_sha,
    ...
    eval_start = time.time()
    y_true, y_pred = _predict(geo_model, eval_loader, device=device, dtype=dtype, parameter_t=best_t.detach())
    rows.append(meta | _metrics(y_true, y_pred) | {
        "mode": "spdim_geodesic",
...
def _dry_run(ep, *, args, bn, TSMNet) -> None:
    ...
    parameter_t = torch.tensor(1.0, dtype=torch.float64, device="cpu")
    y_true, y_pred = _predict(model, loader, device=device, dtype=dtype, parameter_t=parameter_t)
