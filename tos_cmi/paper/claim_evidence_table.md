...
### C14 — DG-relevant deletion EXISTS but is not source-identifiable (measurement→action gap, branch agent/cmi-trace-dg-oracle)
- **Claim:** inverting the objective from "minimize measured leakage" to "minimize source-held-out risk" (subject-CMI demoted to a constraint) shows on real EEGNet that a subject-subspace deletion improving *within-target* generalization DOES exist (mechanism-matched greedy cross-fit oracle, above matched-rank random), yet a source-only selector — including the greedy selector that matches a differentiable supermask's arbitrary-coordinate mechanism — does NOT recover it, and selecting deletions by maximal reduction of a linear within-label subject-decodability proxy is harmful. The gap is source OBSERVABILITY, scoped to the tested bases/selectors — not a universal impossibility theorem.
- **Phase:** DG-erasure oracle + hardened DG-identifiability (Phase-0 cross-fit + Phase-1 4-basis rescue + greedy-source audit). Verified by a 5-skeptic adversarial workflow (high-confidence DOWNGRADE_TO_HINDSIGHT_ONLY).
- **Artifacts:** `results/cmi_trace_dg_identifiability/{dg_identifiability_verdict.json, source_greedy_audit_verdict.json}`; `results/cmi_trace_dg_oracle/dg_oracle_verdict.json` (first pass); `tos_cmi/eval/dg_identifiability.py`; `notes/CMI_TRACE_DG_IDENTIFIABILITY.md`.
- **Numbers (subject/fold-cluster 95% CI):** EXISTENCE greedy cross-fit oracle (cond/full): BNCI2014 +0.021 [+0.012,+0.032] (8/9 folds +, robust), BNCI2015 +0.045 [+0.010,+0.094] (concentrated in 1–2 outlier subjects), matched-rank random −0.015/−0.007; the WEAKER top-k prefix oracle is ~0 (+0.005 [lo −0.000] / +0.001 [lo −0.003]) and ~half the un-cross-fit +0.049/+0.065 is subset-search optimism. IDENTIFIABILITY greedy source-only audit (outer-source-fitted basis + source-LOSO selection): BNCI2014 all bases LCB≤0 (cond −0.0029 [−0.0055,−0.0007], anti-aligned 0.30); BNCI2015 only marg +0.0024 [+0.0013,+0.0036] (≈7% of oracle, unreplicated, align 0.42) = SOURCE_DETECTABLE_TINY. Linear within-label subject-decodability-proxy selector harmful −0.010 [lo −0.019] / −0.063 [lo −0.107]. Ticket subject-leakage certification (CORRECTED cross-fit, F2.0b: eraser-fit basis, split-specific ticket, paired ΔÎ_specific=mean kl(random)−kl(ticket) cluster CI, capacities linear/small/large): NOT certified on either dataset — primary large-capacity ΔÎ_specific = −0.007 [−0.035,+0.027] (BNCI2014), −0.011 [−0.032,+0.011] (BNCI2015), both LCB<0; ticket removes no more validated leakage than matched-rank random (ticket +0.040 vs random +0.047; +0.025 vs +0.036). The earlier "1/4 cells certified" pass is RETRACTED (artifact of a re-selected full-target ticket without eraser-fit split or paired control).
- **Limitation:** two EEGNet MI datasets; the existence ticket is WITHIN-TARGET hindsight (oracle selects on T_select, scores on the same subject's disjoint T_query) — NOT demonstrated cross-subject transfer. "Not source-identifiable" is scoped to the tested bases (marg/cond/rule/grad, full & contested) and selectors (nested prefix + outer-source-fitted greedy source-LOSO). The exact-head-null nullspace result (chain link 2) is on the DGCNN graph representation (stored head), not EEGNet.
- **Status:** SUPPORTED. **Allowed:** "a within-target-beneficial subject-subspace deletion exists but is not recoverable by a source-only selector; a selector maximizing reduction of a linear within-label subject-decodability proxy is harmful for DG; the gap is source observability under the tested bases/selectors." **Forbidden:** "there is no DG-beneficial deletion" (a cross-fitted one exists) · "the deletion is source-identifiable" · "a source-only supermask improves target DG" · "this proves DG-erasure is impossible" (scoped, not universal) · calling the BNCI2015/marg +0.0024 a practical gain (it is ≈7% recovery, unreplicated) · "minimizing the validated CMI is harmful" (the harmful selector optimizes a cheap linear proxy, not the posterior-KL ruler) · attributing the exact-head-null nullspace result to EEGNet (it is DGCNN) · "the DG-beneficial ticket is a certified subject-leakage deletion" (certified vs random in only 1/4 dataset×capacity cells; call it "a deletion within a subject-derived basis").
