...
### C14 — V2 source-only acceptance ceiling / non-identifiability (branch method-deepen-v1, method-deepening; NOT main-paper)
- **Claim:** on a semi-synthetic benchmark (real Lee/Cho EEGNet latents + an injected ground-truth nuisance), a target-beneficial erasure can exist (oracle nuisance removal gives real target gain, +0.06..+0.10 safe / up to +0.46 unsafe), yet the source-only gate cannot certify it: across 144 constructions 0 accept; even SAFE cells with real target gain have source-LOSO benefit LCB ≤ 0 → gate ABSTAINS/REJECTS. Worlds B (task-entangled) REJECT and C (removable-useless) do-not-accept confirm safety + domain-gain≠benefit.
- **Phase:** V2 semi-synthetic (method-deepening). **Fig/Table:** ceiling scatter (source-LOSO benefit LCB vs actual target ΔbAcc LCB).
- **Numbers:** worldA_search 0/144 ACCEPT, max benefit LCB +0.0009; headline safe cell (aligned_noise_flip f=0.15 α=1.0 LEACE) target ΔbAcc +0.097 [LCB +0.067], source-LOSO benefit LCB −0.046 → ABSTAIN.
- **Status:** method-deepening evidence, NOT a main-paper claim (semi-synthetic). **Allowed:** "V2 shows a source-only acceptance ceiling: target-beneficial erasures can exist, but when the benefit is created by a deployment shift not represented in the source domains, source-only evidence cannot safely certify acceptance; the correct action stays reject/abstain." / "crossing the ceiling requires target information or source domains encoding the shift." **Forbidden:** "V2 shows the source-only gate can accept genuine target-beneficial erasure" / "the source-only gate has acceptance power" / "World A is a successful accept case" / "erasure benefit is certifiable from source-only data" / "source-only benefit is impossible in general" (too strong — it is conditional non-identifiability) / presenting any oracle diagnostic as a deployable method result.
