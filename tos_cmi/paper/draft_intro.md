# §1 Introduction (draft — written to reflect the honest result, not the original algorithmic ambition)
...
A popular remedy is **conditional invariance** — penalizing the dependence of the representation on the
...
These failures suggest a question that is logically prior to *controlling* leakage: **where does the leakage
...
to cross-subject error. We therefore study conditional invariance as a **measurement-and-certification**
problem rather than an always-on regularizer.

We present TOS-CMI, a four-layer pipeline that (i) **localizes** a domain-rich, task-light subspace with a
conditional score-Fisher operator; (ii) **deletes** it with a direct-sum projector that algebraically
preserves the task carrier; (iii) **gates** the deletion with a conditional task-risk certificate estimated
by a cross-fitted plug-in log-ratio test with a power floor; and (iv) **refuses** — returns the identity
...
framework exposes a **gap between measuring leakage and controlling it**. In a high-dimensional SPD/LogEig
...
**Contributions.** (1) A measurement-to-certified-control framework for conditional domain leakage in EEG,
...
domain leakage is **measurable, sometimes low-rank removable (representation- and capacity-dependent), yet
not a sufficient control target for cross-subject generalization**. In three sentences: *measurement works;
