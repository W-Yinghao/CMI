...
(\cite{barachant2012multiclass,zanini2018transfer,yger2017riemannian}), and batch/domain alignment layers such as the SPD
batch-normalization in TSMNet target calibration-free cross-subject decoding (\cite{huang2017spdnet,kobler2022spd});
convolutional architectures such as EEGNet and ShallowConvNet are standard decoders
(\cite{lawhern2018eegnet,schirrmeister2017deep}). These methods aim to \emph{produce} an invariant or aligned representation.
...
conditionally on the label: adversarial domain removal (\cite{ganin2016dann,long2018cdan}), moment matching
(\cite{sun2016deepcoral,gretton2012mmd,long2015dan}), invariance penalties (\cite{arjovsky2019irm,krueger2021rex}), and conditional-mutual-information /
LPC-style penalties that target $I(Z;D|Y)$ (\cite{belghazi2018mine,mukherjee2020ccmi,cha2022miro}). These are typically
...
(\cite{ravfogel2020inlp,ravfogel2022rlace,belrose2023leace}) --- and more broadly as subspace/feature removal. These methods typically
\emph{erase} a targeted concept and report that it is no longer (linearly) decodable; a closely related
line studies task-preserving (oblique) concept removal \cite{holstege2025taskpreserving}.
...
(\cite{geifman2017selective,geifman2019selectivenet,shafer2008conformal}), and causal/effect
estimation uses cross-fitted, one-step (doubly-robust) estimators with explicit power considerations
(\cite{chernozhukov2018doubleml}).
