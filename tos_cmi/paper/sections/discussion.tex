...
mean LOSO target accuracy is flat-to-worse and uncorrelated with the amount of leakage removed. A direct target-deployment test of the frozen erasers (\S4.5, Table~\ref{tab:target_deploy}) reaches the same conclusion: none --- including the optimal linear eraser --- improves target accuracy, and the only NLL movement is reproduced by random removal. Two
