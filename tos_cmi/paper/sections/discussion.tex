...
(and deploying the frozen selective erasers on the target, \S4.5, gives no accuracy gain either). The common thread is a gap between \textbf{localizing/measuring}
...
mean LOSO target accuracy is flat-to-worse and uncorrelated with the amount of leakage removed. A direct target-deployment test of the frozen erasers (\S4.5, Table~\ref{tab:target_deploy}) reaches the same conclusion: none --- including the optimal linear eraser --- improves target accuracy, and the NLL movement is not erasure-specific (same-sign under random removal, and INLP's NLL drop coincides with task collapse). Two
...
\begin{itemize}
\item \textbf{Scope of validation.} The primary mechanism figures are on BCI-IV-2a, but the target-deployment
  conclusion is validated on four additional real EEG datasets (2b, Lee2019, Cho2017, High-Gamma; $129$ further
  subjects; \S4.7). The validation is not exhaustive over all EEG paradigms, montages, or model families, and
  2b-TSMNet is excluded because the $3$-channel montage makes the high-dimensional SPD tangent diagnostic degenerate.
...
\item \textbf{Dimension-vs-type: largely capacity, with a residual architecture effect at high $d_z$.} The original
  ...
  explained by capacity. Remaining caveats: the factorial is run on BCI-IV-2a only, and the LDA cap from $8$ source subjects.
