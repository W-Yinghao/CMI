...
representation; in a compact convolutional latent the leakage \emph{is} low-rank removable (linearly), yet
removing it does not improve target accuracy. The common thread is a gap between \textbf{localizing/measuring}
...
\item \textbf{Dimension-vs-type: provisionally addressed by capacity factorial.} The original two-point comparison confounded
  architecture with latent dimension (SPD/$210$ vs conv/$16$). The architecture$\times$dimension factorial
  (\S4.5, Fig.~\ref{fig:capacity}) indicates this (seed-0 factorial; multi-seed hardening in progress): the nonlinear erasure residual is a function of latent
  \emph{dimension}, with SPD and convolutional latents nearly coinciding at matched $d_z$ --- a seed-0 trend consistent
  with a capacity-mediated (not representation-type) contrast, to be confirmed multi-seed. Remaining caveats on this factorial: a single dataset
  (BCI-IV-2a), seed~0 (multi-seed in progress), and the LDA cap from $8$ source subjects.
...
multi-seed and multi-dataset hardening of the capacity-mediated verdict, source-OOD benefit gating on a large-$N$ dataset, and folding the certificate into end-to-end training with
