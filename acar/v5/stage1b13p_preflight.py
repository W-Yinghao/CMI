"""ACAR V5 Stage-1B13P metadata/header/repair/name/geometry PREFLIGHT (read-only; NOT a Stage-1B build).

Extends Stage-1B12P with the reviewed channels.tsv channel-NAME repair (ds003944/ds003947). For every recording of the 7 frozen DEV
cohorts it: plans the reviewed BrainVision repair (marker synth / pointer rewrite / channels.tsv rename), materializes the ephemeral
repaired header into a staging dir, opens the (repaired-or-original) header at preload=False, adjudicates duplicates, computes the
...
def run():
    ...
    staging = tempfile.mkdtemp(prefix="acar_v5_1b13p_staging_")
