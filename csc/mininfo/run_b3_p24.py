...
def _one_cell(kind, scen_name, scen, m, cluster_seed, n_subjects, n_boot):
    ...
    return dict(scenario=scen_name, cluster_seed=int(cluster_seed), kind=kind, truth=truth,
                ...
                fixed_margin_null_p=log.get("fixed_margin_null_p"), standard_null_p=log.get("standard_null_p"),
                would_confirm_under_standard_null=log.get("would_confirm_under_standard_null", False),
                studentized_p_value=log.get("studentized_p_value"), studentized_stat=log.get("studentized_stat"),
                subject_consistency_lcb=log.get("subject_consistency_lcb"), mean_delta=log.get("mean_delta"),
                sd_delta=log.get("sd_delta"), se_delta=log.get("se_delta"),
                n_subject_deltas=log.get("n_subject_deltas"),
                alpha_family=log.get("alpha_family"), alpha_budget=log.get("alpha_budget"),
                lcb_level=log.get("lcb_level"), subject_consistency_lcb_budget=log.get("subject_consistency_lcb_budget"),
                old_p24c_decision=log.get("old_p24c_decision"), new_p24d_decision=log.get("new_p24d_decision"),
                old_decision_without_studentized_gate=log.get("old_decision_without_studentized_gate"),
                new_decision_with_studentized_gate=log.get("new_decision_with_studentized_gate"),
                null_version=log.get("null_version"), n_sampler_failures=log.get("n_sampler_failures", 0),
                margin_preserved=log.get("margin_preserved"),
