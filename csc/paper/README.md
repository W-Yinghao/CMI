...
- **Fig 3** `figures/fig3_routeA_negative_B3_positive.png` — combined frozen-confirmatory evidence:
  Route A (left) FAILS both endpoints vs Route B3 (right) PASSES both, on the two pre-registered endpoints
  (false-cert rate vs α; power vs 0.50 bar). Referenced from both `routeA.tex` (panel a) and
  `routeB3_confirmatory.tex` (`fig:evidence`). Generated by `csc/tools/make_fig3_evidence.py`, which
  **loads and asserts every number from the committed artifacts** (`confirmatory.json`,
  `b3_confirmatory_result.json`). The old `fig3_dev_vs_confirmatory.png` is superseded (retained on disk).
Other raster figures regenerate/extend via `csc/tools/make_paper_figures.py`.
