...
| A10 | **A dual-disease V6 label-free router is justified by action-viability diagnostics** (label-free features reliably predict the beneficial action-sign on BOTH PD and SCZ under the pre-registered V6-A0 continuation gate) | **REFUTED → `V6_STOP`** | v6-A0b real exploratory diagnostic (impl `ea37fc2`, run `28588ac`, SLURM 887279, EVAL-only, labels ONLY for ΔR_a + f_0 calibration). **PD passes all four sub-gates** (oracle_red_upper 0.413, coverage_subject_macro 0.796, sign_auroc_subject_balanced 0.705, perm_p 0.001; per-action t3a AUROC 0.902); **SCZ fails ONLY `perm_p_subject_block`** (sign_auroc 0.632 clears 0.60 but `p=0.208` — not significant vs a subject-block null). The null is EVALUABLE, not underpowered (PD 229/230 permutable, SCZ 221/226; 1000 valid perms each). V6_CONTINUE needs both diseases → `V6_STOP`. Consequence: **no V6 protocol draft, no candidate selection, no policy fitting, no external, lockbox sealed** — a NEW dated protocol only. Closeout: `notes/ACAR_V6_A0_CLOSEOUT.md`, result `notes/ACAR_V6_A0_ACTION_VIABILITY_AUDIT_RESULT.md` |
| A11 | **A policy-independent action BENEFIT ENVELOPE exists on PD and SCZ DEV** (an oracle that per-batch picks the best action-or-noop achieves substantial NLL reduction; some batches genuinely benefit) | **SUPPORTED (DIAGNOSTIC_ONLY — NOT deployable)** | v6-A0b: `oracle_red_upper` = 0.413 (PD) / 0.585 (SCZ) nats; `beneficial_coverage_subject_macro` = 0.796 / 0.729; `oracle_conditional_harm = 0`. So V5's DEV_STOP was NOT "actions always harmful" — the benefit exists; what fails (A10) is finding it label-free on both diseases. `matched_coral` is ~neutral (mean ΔR ≈ 0); `spdim`/`t3a` are on-average catastrophic. **Scope: an ORACLE (label-aware) envelope; does NOT imply a deployable label-free router — read WITH A10, never alone.** |
...
**ACAR v6-A0 direction (EXECUTED → `V6_STOP`; `notes/ACAR_V6_A0_CLOSEOUT.md`, result `…AUDIT_RESULT.md`, plan `…AUDIT_PLAN.md`).**
V6-A0 asked *"is any adaptation action worth routing?"* before writing a router. The action-viability audit was implemented
(V6-A0a `bd6b2ed`), subject-cluster-corrected (V6-A0a2 `ea37fc2`), and RUN as the real exploratory label-consuming diagnostic
(V6-A0b `28588ac`). **Outcome = `V6_STOP` (A10).** The pre-registered gate (oracle_red_upper>0.02 AND beneficial_coverage_subject_
macro≥0.15 AND sign_auroc_subject_balanced≥0.60 AND perm_p_subject_block≤0.05, BOTH diseases): **PD passes all four** (sign-AUROC
0.705, p=0.001; t3a per-action AUROC 0.902), **SCZ fails only perm_p** (AUROC 0.632 but p=0.208 vs a well-powered subject-block
null). A benefit envelope EXISTS on both (A11: oracle_red_upper 0.41/0.59, coverage 0.80/0.73) — V5's DEV_STOP was "router can't
find the beneficial cases", not "actions always harmful" — but a **dual-disease label-free router is not justified**. The contingent
V6 pivot (identity-dominant bounded logit calibration) is therefore NOT triggered and NOT authorized. Firewall held: the diagnostic
only DECIDED not to draft V6; it fitted no policy, selected no candidate, and touched no external/lockbox. Any continuation is a NEW
dated protocol (a new hypothesis, not a "V6 continuation") with a clean confirmation route on a still-sealed substrate; the DEV
labels have now been used extensively for diagnosis, so anything designed from these findings is exploratory until so confirmed.
