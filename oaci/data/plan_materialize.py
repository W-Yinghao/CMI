...
def _sampling_design_hash(pop_sig, role, structure_hash, cells, per_cell, micro_size, replacement_mode, idx) -> str:
    h = hashlib.sha256()
    ...
    h.update(f"|k={per_cell}|mb={micro_size}|rep={replacement_mode}".encode())
    return h.hexdigest()
...
def _materialize_alignment(sampler, idx, pop_sig, role, structure_hash, cells, per_cell, micro_size,
                           replacement_mode, warmup_steps, total_inner, critic_steps) -> AlignmentPlan:
    t = 0
    warmup = []
    for _ in range(warmup_steps):
        warmup.append(_logical_from_step(sampler, t, micro_size)); t += 1
    game = []
    for _ in range(total_inner):
        cbs = tuple(_logical_from_step(sampler, t + c, micro_size) for c in range(critic_steps))
        t += critic_steps
        enc = _logical_from_step(sampler, t, micro_size); t += 1
        game.append(AlignmentGameStep(cbs, enc))
    warmup = tuple(warmup); game = tuple(game)
    sdh = _sampling_design_hash(pop_sig, role, structure_hash, cells, per_cell, micro_size, replacement_mode, idx)
    return AlignmentPlan(warmup, game, pop_sig, _alignment_plan_hash(pop_sig, warmup, game),
...
def materialize_oaci_alignment_plan(idx: UnitIndex, support_graph, pop_sig, warmup_steps, total_inner,
                                    critic_steps, per_cell, micro_size, base_seed, replacement_mode="auto") -> AlignmentPlan:
    ...
    return _materialize_alignment(sampler, idx, pop_sig, "oaci_alignment", support_graph.support_hash(),
                                  sampler.cells, per_cell, micro_size, replacement_mode,
...
def materialize_full_domain_alignment_plan(idx: UnitIndex, pop_sig, warmup_steps, total_inner,
    ...
    return _materialize_alignment(sampler, idx, pop_sig, "full_domain_alignment", struct,
                                  cells, per_cell, micro_size, replacement_mode,
                                  warmup_steps, total_inner, critic_steps)
