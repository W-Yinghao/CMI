#!/usr/bin/env bash
#SBATCH --job-name=c84l1p-regression
...
#SBATCH --output=/home/infres/yinwang/CMI_AAAI/c84l1p_regression_logs/%x-%j.out
#SBATCH --error=/home/infres/yinwang/CMI_AAAI/c84l1p_regression_logs/%x-%j.err
set -euo pipefail
...
if [[ "$head_commit" != "$remote_commit" ]]; then
  echo "C84L1P regression refused: HEAD != origin/oaci" >&2
  exit 3
fi
if [[ -n "$(git status --porcelain)" ]]; then
  echo "C84L1P regression refused: worktree is not clean" >&2
  exit 4
fi

mapfile -t tests < <("$python" -m oaci.multidataset.c84r_regression_suite "$suite")
if [[ "${#tests[@]}" -eq 0 ]]; then
  echo "C84L1P regression refused: empty suite" >&2
  exit 5
fi

export CUBLAS_WORKSPACE_CONFIG=:4096:8
export PYTHONHASHSEED=0
export PYTHONPYCACHEPREFIX="/tmp/c84l1p-reg-${suite}-${SLURM_JOB_ID}/pycache"
...
printf 'C84L1P_REGRESSION suite=%s commit=%s python=%s CPU=48 GPU=0\n' \
  "$suite" "$head_commit" "$python"
"$python" -m pytest -q -rs \
  --basetemp="/tmp/c84l1p-pytest-${suite}-${SLURM_JOB_ID}" \
  -o "cache_dir=/tmp/c84l1p-pytest-cache-${suite}-${SLURM_JOB_ID}" \
  "${deselect[@]}" "${tests[@]}"
