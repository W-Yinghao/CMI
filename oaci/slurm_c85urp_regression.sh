#!/usr/bin/env bash
#SBATCH --job-name=c85urp-regression
...
#SBATCH --output=/home/infres/yinwang/CMI_AAAI/c85urp_regression_logs/%x-%j.out
#SBATCH --error=/home/infres/yinwang/CMI_AAAI/c85urp_regression_logs/%x-%j.err
set -euo pipefail
...
if [[ "$head_commit" != "$remote_commit" ]]; then
  echo "C85URP regression refused: HEAD != origin/oaci" >&2
  exit 3
fi
if [[ -n "$(git status --porcelain)" ]]; then
  echo "C85URP regression refused: worktree is not clean" >&2
  exit 4
fi
...
if [[ "${#tests[@]}" -eq 0 ]]; then
  echo "C85URP regression refused: empty suite" >&2
  exit 5
fi

export PYTHONHASHSEED=0
export PYTHONPYCACHEPREFIX="/tmp/c85urp-reg-${suite}-${SLURM_JOB_ID:-local}/pycache"
...
printf 'C85URP_REGRESSION suite=%s commit=%s python=%s CPU=48 GPU=0\n' \
  "$suite" "$head_commit" "$python"
"$python" -m pytest -q -rs \
  --basetemp="/tmp/c85urp-pytest-${suite}-${SLURM_JOB_ID:-local}" \
  -o "cache_dir=/tmp/c85urp-pytest-cache-${suite}-${SLURM_JOB_ID:-local}" \
  "${deselect[@]}" "${tests[@]}"
