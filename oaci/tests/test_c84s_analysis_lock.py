...
def test_frozen_protocol_and_complete_field_replay() -> None:
    ...
    assert replay["hashes"]["end_to_end_repair"] == runtime.EXPECTED["end_to_end_repair"]
    assert replay["hashes"]["frontier_stability"] == runtime.EXPECTED["frontier_stability"]
    assert replay["hashes"]["historical_lock"] == runtime.EXPECTED["historical_lock"]
...
def test_static_process_isolation_passes() -> None:
    ...
    assert len(checks) == len(runtime.IMPLEMENTATION_PATHS) + 4
...
def test_execution_lock_when_present_binds_scope() -> None:
    ...
    assert lock["schema_version"] == "c84s_analysis_execution_lock_v2"
    assert lock["historical_lock_supersession"]["preserved"] is True
    assert lock["historical_lock_supersession"]["operative_for_future_C84S"] is False
