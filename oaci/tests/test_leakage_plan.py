...
from oaci.leakage import (CriticConfig, FrozenFeatures, LeakageBootstrapPlan, LeakageScoreCache,
                          LeakageScoreKey, bootstrap_ucb, critic_config_hash, frozen_feature_hash,
                          make_fold_plan, make_leakage_bootstrap_plan, make_leakage_design)
...
def _bundle(seed=0):
    ...
    feat = FrozenFeatures(Z=X, y=y, d=d, group=g)
    ...
    fold = make_fold_plan(feat, sg, n_folds=4, seed=0)
...
def test_fold_plan_hash_is_order_invariant():
    ...
    feat2 = FrozenFeatures(Z=X[perm], y=y[perm], d=d[perm], group=g[perm])
