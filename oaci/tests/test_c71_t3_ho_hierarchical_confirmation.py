"""C71 T3-HO confirmation readiness tests."""
...
def test_c71_taxonomy_cli_auth_and_noauth_gate():
...
def test_c71_protocol_timing_parent_sha_and_no_t3_access():
    ...
    assert protocol["authorization_token_status"] == "absent"
    assert protocol["t3_ho_cache_generation_authorized"] == 0
    ...
    assert timing["first_t3_ho_manifest_path_read"]["timestamp_utc"] == ""
    assert timing["first_t3_ho_manifest_path_read"]["status"] == "not_accessed_no_authorization"
    assert timing["first_t3_ho_outcome_read"]["timestamp_utc"] == ""
    assert timing["first_t3_ho_outcome_read"]["status"] == "not_accessed_no_authorization"


def test_c71_required_readiness_tables_and_view_contracts():
    ...
    assert all(r["path"] == "" and r["sha256"] == "" for r in views.values())
    ...
    assert {r["status"] for r in hypotheses} == {"not_run_not_authorized"}
...
def test_c71_inference_contracts_feature_provenance_and_failure_ledger():
    ...
    assert {r["labels_counted_as"] for r in unique} == {"unique_target_trial_ids_per_class"}
    ...
    assert {r["bootstrap"] for r in boot} == {"checkpoint_cluster", "trial_id_cluster", "crossed_pigeonhole", "target_cluster"}
    ...
    assert failures["missing_exact_cli_authorization"]["status"] == "blocking_execution_not_readiness"
    assert failures["protocol_locked_before_t3_access"]["status"] == "pass"
    assert failures["t3_ho_not_consumed"]["status"] == "pass"
