...
def aggregate_loso(fold_results, *, subjects=SUBJECTS, protocol_hash=None) -> dict:
    ...
    phashes = {f.get("protocol_hash") for f in folds}
    if protocol_hash is not None and phashes != {protocol_hash}:
        raise ValueError(f"folds disagree on the protocol hash: {phashes}")
    for f in folds:
    ...
    return {"n_folds": len(folds), "targets": targets, "protocol_hash": (protocol_hash or next(iter(phashes))),
...
def read_fold_artifact(artifact_dir, target) -> dict:
    ...
    return {"target": int(target), "artifact_dir": artifact_dir,
            ...
            "protocol_hash": marker.get("context_hash"),
