"""Lock-bound C84S V4 coordinator using immutable V3 Stage-A views."""
...
from .c84sr2_common import DEFAULT_OUTPUT_ROOT
from .c84sr2_runtime_guard import consume_authorization, pre_label_access_guard
...
def _run_stage(
    *, name: str, module: str, arguments: Sequence[str], run_root: Path,
    ...
    receipt = {
        "schema_version": "c84sr2_subprocess_receipt_v1", "stage": name,
        "module": module, "command": command, "started_at_unix_ns": started,
        "finished_at_unix_ns": time.time_ns(), "exit_code": completed.returncode,
...
def run_real(*, authorization_path: Path, output_root: Path) -> dict[str, Any]:
    ...
    lifecycle_path = run_root / "C84S_V4_LIFECYCLE_ATTEMPT.json"
    lifecycle: dict[str, Any] = {
        "schema_version": "c84sr2_lifecycle_attempt_v1", "status": "STARTED",
    ...
    write_json(run_root / "C84S_V4_PRE_LABEL_ACCESS_REPLAY.json", {
        "schema_version": "c84sr2_pre_label_access_replay_v1",
    ...
    try:
        ...
        write_json(lifecycle_path, lifecycle)
        stage_a = _run_stage(
            name="Stage_A_Replay", module="oaci.multidataset.c84sr2_stage_a_replay",
            arguments=(
        ...
        require(replay["status"] == "PASS" and replay["label_loader_calls"] == 0 and
                ...
                "immutable Stage-A replay contract failed")
        ...
        require(sha256_file(construction_handoff) == replay["construction_handoff_sha256"],
                "replayed construction handoff drift")
        require(sha256_file(evaluation_seal) == replay["evaluation_seal_sha256"],
                "replayed evaluation seal drift")
        ...
        require(freeze["status"] == "SELECTION_FROZEN_EVALUATION_DESCRIPTOR_NOT_YET_AVAILABLE" and
                ...
                freeze["contexts"] == 944 and freeze["Q0_records"] == 9110448,
                "Stage-B exact freeze failed before evaluation release")
        ...
        write_json(lifecycle_path, lifecycle)
        final = {
            "schema_version": "c84sr2_real_execution_result_v1", "status": "COMPLETE",
            "final_gate": result["primary_gate"],
            "label_frontier_tag": result["label_frontier_tag"],
        ...
        write_json(run_root / "C84S_V4_EXECUTION_RESULT.json", final)
        return final
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_real(authorization_path=args.authorization_record, output_root=args.output_root)
