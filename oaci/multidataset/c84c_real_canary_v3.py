"""C84C V4 engineering canary with repaired float32 linear replay."""
...
from .c84r_montage_repair import INTERFACE_ID, MONTAGE_SHA256
...
class C84CCanaryV3Error(runtime.C84R3RuntimeError):
    """Raised when the C84C V4 replacement engineering canary fails."""
...
def _instrument_and_replay(
    ...
    source_logits, _ = base._forward_model(model, source_audit.X, torch)
    target_logits, target_z = base._forward_model(model, target.X, torch)
...
def _configure_base() -> None:
    ...
    base.C84CCanaryV2Error = C84CCanaryV3Error
    base._instrument_and_replay = _instrument_and_replay
    ...
    *,
    ...
    try:
        ...
        if not torch.cuda.is_available() or os.environ.get("SLURM_JOB_ID") is None:
            raise C84CCanaryV3Error("C84C V4 requires an authorized Slurm CUDA allocation")
        torch.use_deterministic_algorithms(True, warn_only=False)
        if not torch.are_deterministic_algorithms_enabled():
            raise C84CCanaryV3Error("torch deterministic algorithms are not enabled")
        ...
        datasets = []
        for dataset in base.protocol.DATASET_ORDER:
        ...
        manifest = {
            "schema_version": "c84c_complete_canary_manifest_v3",
...
def synthetic_schema_dry_run() -> dict[str, Any]:
    ...
    return {
        **payload,
        "schema_version": "c84c_schema_dry_run_v3",
        "linear_replay_abs_tolerance": runtime.LINEAR_REPLAY_ABS_TOLERANCE,
        "strict_identity_abs_tolerance": runtime.STRICT_IDENTITY_ABS_TOLERANCE,
        "failed_attempt_reused": False,
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="C84C V4 repaired engineering canary")
    ...
    if args.command == "show-contract":
        print(json.dumps({
            "stage": "C84C", "protocol": str(CANARY_PROTOCOL_PATH),
            "lock": str(EXECUTION_LOCK_PATH), "fresh_authorization_required": True,
            "linear_replay_abs_tolerance": runtime.LINEAR_REPLAY_ABS_TOLERANCE,
            "strict_identity_abs_tolerance": runtime.STRICT_IDENTITY_ABS_TOLERANCE,
            "failed_attempt_reusable": False, "target_y_access": 0,
        }, sort_keys=True))
        return 0
